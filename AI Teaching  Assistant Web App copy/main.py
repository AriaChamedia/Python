...
st.write("Ask me anything about various subjects and I will provide you with a helpful response!")

#Converts chat history into a downloadable text file
#This function is used to convert the chat history into a downloadable text file.
def export_bytes(history):
    text = "".join([f"Q{i}: {h['question']}\nA{i}: {h['answer']}\n\n" for i, h in enumerate(history, 1)])
    return io.BytesIO(text.encode("utf-8"))

st.session_state.setdefault("history", [])
column_clear, column_export= st.columns([1,2])
with column_clear:
...
user_input=st.text_input("Ask any question about various subjects: ") 
if st.button("Ask"):
    q=user_input.strip()
    if q:
